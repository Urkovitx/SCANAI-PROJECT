# -*- coding: utf-8 -*-
r"""
ROBOT_SCAN_1.2 (Versió Fusionada)
==================================
Aquesta versió combina la interfície d'usuari i el flux de treball per lots de ROBOT_SCAN_1.2
amb el motor d'anàlisi més potent i modular de ROBOT_SCAN_1.6, eliminant la dependència d'Adobe.
Objectiu: El millor dels dos mons. La UI intuïtiva amb la màxima potència de detecció.
"""
...
try:
    ...
    _HAS_TESS = False
    st.warning(
        "⚠️ `pytesseract` no està instal·lat. L'OCR sobre imatges no funcionarà. "
        "Pots instal·lar-ho amb `pip install pytesseract`."
    )
try:
    import fitz  # PyMuPDF
    HAVE_PYMUPDF = True
...
try:
    ...
    HAVE_CV = False
    st.error(
        "❌ Falta la llibreria per al processament d'imatge: **OpenCV**.\n\n"
...
try:
    ...
    HAVE_SPACY = False
    st.warning(
        "⚠️ `spaCy` o el model `es_core_news_sm` no està instal·lat. "
        "Instal·la-ho amb `pip install spacy` i `python -m spacy download es_core_news_sm`."
    )
try:
    ...
    HAVE_SPEECH = False
    st.warning(
        "⚠️ `speech_recognition` no està instal·lat. "
        "Instal·la-ho amb `pip install SpeechRecognition pyaudio`."
    )
...
def convert_spanish_numbers_to_digits(text: str) -> str:
    ...
    # Diccionari de números en espanyol (només dígits individuals 0-9)
    number_words = {
    ...
    # Converteix el text a majúscules i divideix en paraules
    words = text.upper().split()
    result = []

    for word in words:
        # Neteja la paraula de puntuació
        clean_word = re.sub(r'[^\w]', '', word)
        if clean_word in number_words:
            result.append(number_words[clean_word])
        elif clean_word.isdigit():
            # Si ja és un dígit, l'afegeix directament
            result.append(clean_word)
        # Si no és un número, l'ignora (no l'afegeix al resultat)

    # Retorna els dígits units sense espais
    return ''.join(result)

def get_voice_input() -> str:
    """Captura entrada de veu i retorna el text reconegut."""
    if not HAVE_SPEECH or not sr:
        st.error("Speech recognition no disponible.")
        return ""
    recognizer = sr.Recognizer()
    with sr.Microphone() as source:
        st.info("Escoltant... Parla ara.")
        try:
            audio = recognizer.listen(source, timeout=5)
            text = recognizer.recognize_google(audio, language='es-ES')  # Canvia a 'ca-ES' si cal
            # Converteix números en paraules a dígits
            converted_text = convert_spanish_numbers_to_digits(text)
            return converted_text.upper()
        except sr.WaitTimeoutError:
            st.warning("No s'ha detectat cap so.")
...
def organitzar_pdfs(directori_principal: str):
    """
    Busca fitxers 'Page00000000.pdf' dins de subdirectoris, els mou al
    directori principal (parent), canviant el seu nom pel del subdirectori, i
    esborra el subdirectori original. Finalment, esborra el directori_principal.
    Args:
    directori_principal: La ruta a la carpeta principal (ex: 'C:/.../IMPORT_ESCAN').
    """
    # Convertim el string de la ruta a un objecte Path per a més seguretat
    # i compatibilitat entre sistemes operatius.
    ruta_base = Path(directori_principal)
    if not ruta_base.is_dir():
        print(f"Error: El directori '{directori_principal}' no existeix o no és una carpeta.")
        return
    print(f"Processant directori: '{ruta_base}'")
    # 1. Obtenim una llista de tots els subdirectoris abans de començar a modificar res.
    # Això evita problemes si la llista de carpetes canvia mentre iterem sobre ella.
    subdirectoris = [element for element in ruta_base.iterdir() if element.is_dir()]
    # 2. Ara iterem sobre la llista estàtica que hem creat.
    for element in subdirectoris:
        if element.is_dir():
            subdirectori_path = element
            nom_subdirectori = element.name
            print(f"\nProcessant subcarpeta: '{nom_subdirectori}'")
            # Construïm la ruta completa al fitxer PDF que busquem
            fitxer_pdf_origen = subdirectori_path / 'Page00000000.pdf'
            # Comprovem si el fitxer PDF existeix dins del subdirectori
            if fitxer_pdf_origen.is_file():
                # Construïm la nova ruta de destí, canviant el nom del fitxer
                # pel nom de la carpeta i afegint l'extensió .pdf
                nou_nom_fitxer = f"{nom_subdirectori}.pdf"
                fitxer_pdf_desti = ruta_base.parent / nou_nom_fitxer
                # Movem el fitxer PDF al directori principal amb el nou nom
                shutil.move(fitxer_pdf_origen, fitxer_pdf_desti)
                print(f" -> Mogut i canviat de nom a: '{fitxer_pdf_desti}'")
                try:
                    # Un cop mogut el fitxer, esborrem la carpeta que ara està buida
                    # shutil.rmtree() esborra una carpeta i tot el seu contingut.
                    # És més segur que os.rmdir() si per error hi hagués algun altre fitxer.
                    shutil.rmtree(subdirectori_path)
                    print(f" -> Esborrat subdirectori: '{subdirectori_path}'")
                except Exception as e:
                    print(f" !! Error esborrant el subdirectori '{nom_subdirectori}': {e}")
            else:
                print(f" -- Avís: No s'ha trobat 'Page00000000.pdf' a '{nom_subdirectori}'.")
    # 3. Neteja final: esborrem qualsevol subdirectori que hagi pogut quedar.
    # Això assegura que la carpeta quedi completament neta de subcarpetes.
    print("\nRealitzant neteja final de subdirectoris...")
    subdirectoris_restants = [element for element in ruta_base.iterdir() if element.is_dir()]
    if not subdirectoris_restants:
        print(" -> No queden subdirectoris per esborrar.")
    for subdirectori in subdirectoris_restants:
        try:
            shutil.rmtree(subdirectori)
            print(f" -> Esborrat subdirectori restant: '{subdirectori}'")
        except Exception as e:
            print(f" !! Error en la neteja final de '{subdirectori}': {e}")
    # 4. Esborrar el directori_principal si està buit
    try:
        shutil.rmtree(ruta_base)
        print(f" -> Esborrat directori principal: '{ruta_base}'")
    except Exception as e:
        print(f" !! Error esborrant el directori principal '{ruta_base}': {e}")
    print("\nProcés finalitzat.")
...
# =============================
# ESTRUCTURES DE DADES (del motor 1.6)
# =============================
@dataclass
class BarcodeHit:
    symbology: str
...
# =============================
# HELPERS (normalització + OCR)
# =============================
def _preprocess_image(pil_img: Image.Image) -> Image.Image:
    """
    Aplica una sèrie de filtres de preprocessament a una imatge per millorar la qualitat de l'OCR.
    1. Converteix a escala de grisos.
    2. Escalat si és petit.
    3. Unsharp + autocontrast.
    4. Binarització simple.
    """
    g = pil_img.convert('L')
    w, h = g.size
    if max(w, h) < 1400:
...
def _norm(s: str) -> str:
    ...
    s = re.sub(r"[\u2010-\u2015]", "-", s)  # diferents guions tipogràfics -> '-'
...
def ocr_image(img: Image.Image, lang: str = 'spa+eng') -> str:
    ...
    if not _HAS_TESS:
        ...
        return ''
    try:
        # Tesseract per a imatges sense text digital
...
def _norm_number(s: str) -> str:
    ...
    if not t:
        return ""
    # Converteix formats amb milers/punt o coma decimal
    if "," in t and "." in t:
        if t.rfind(",") > t.rfind("."):
            t = t.replace(".", "").replace(",", ".")
        else:
...
# =============================
# DETECCIÓ + EXTRACTORS + AVISOS + BARCODES
# =============================
def enhance_extraction_with_spacy(text: str) -> Dict[str, List[str]]:
    """Utilitza spaCy per detectar tokens relacionats amb 'delivery', 'ship_to', 'CTN', 'loop 371' i extreure números associats."""
